...
public class UIUpdateUserRelationship extends UIContainer {

  private Identity identity = null;
  private Relationship relationship;

  public UIUpdateUserRelationship() {
  }

  public UIUpdateUserRelationship setIdentity(Identity identity) {
    this.identity = identity;
    return this;
  }

  public UIUpdateUserRelationship setRelationship(Relationship relationship) {
  ...
  public void processRender(WebuiRequestContext context) throws Exception {
    ...
    if(identity != null) {
      ...
      String label = res.getString("UIAllPeople.label.Connect");
      ...
      if (status == Type.PENDING && relationship.isSender(identity)) {
        statusLabel = res.getString("UIAllPeople.label.InvitationReceived");
        isShowConfirm = true;
      }
      //
      ...
      context.getJavascriptManager().getRequireJS().require("SHARED/socialUtil", "utils")
             .addScripts("utils.updateUserRelationship('" + identity.getId() + "','" + isShowConfirm + "' );");
    }
    identity = null;
    writer.append("</div>");
  }
